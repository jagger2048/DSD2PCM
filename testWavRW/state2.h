...
extern "C" {
	...
	void state2_init(FIR* handles, float *half_coeffs, unsigned int half_order) {
		int k = 1;
	...
	FIR * state2_create(unsigned int half_order, float *half_coeffs) {
		FIR * handles = (FIR *)malloc(sizeof(FIR));
		if (handles != NULL)
		{
			state2_init(handles, half_coeffs, half_order);
		}
		return handles;
	}
	int state2_process(FIR *handles, float *in, unsigned int input_offest, float *out, unsigned int out_offest, unsigned int nSample, unsigned nStep) {
	...
	int state2_destory(FIR* handles) {
		if (handles != NULL)
		{
			free(handles->buffer);
